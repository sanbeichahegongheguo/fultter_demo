...
class RoomRecordingPage extends StatefulWidget {
  static final String sName = "RoomRecordingPage";
  final RoomData roomData;
  final String token;
  const RoomRecordingPage({Key key, this.roomData, this.token}) : super(key: key);
  @override
  State<StatefulWidget> createState() {
    return RoomRecordingPageState(roomData);
  }
}

class RoomRecordingPageState extends State<RoomRecordingPage> with SingleTickerProviderStateMixin, LogBase {
  ...
  RoomRecordingPageState(RoomData roomData) {
  ...
  Widget build(BuildContext context) {
    var coursewareWidth = (ScreenUtil.getInstance().screenWidth * 1) - MediaQuery.of(context).padding.left;
...
//                               )
                            ...
                            Positioned(
                              bottom: ScreenUtil.getInstance().getHeight(16),
                              right: ScreenUtil.getInstance().getWidth(11.2),
                              child: _buildVideo(),
                            ),
        ...
        javascriptChannels: <JavascriptChannel>[
          JavascriptChannel(
              ...
              onMessageReceived: (JavascriptMessage message) async {
                ...
                Log.f("@收到Courseware 回调 $msg", tag: RoomRecordingPage.sName);
...
//    });
  ...
  Widget _buildVideo() {
    FijkView result = FijkView(
      width: ScreenUtil.getInstance().screenWidth * 0.17,
      height: ScreenUtil.getInstance().getHeightPx(530),
...
//      }
  ...
  Future<void> initializeRtm() async {
    ...
    Store<GSYState> store = StoreProvider.of(context);
    Log.d("Store<GSYState> ${store.state.userInfo.toJson()}", tag: RoomRecordingPage.sName);
    try {
...
//      element.playTime = element.t - courseRecordData.startTime + 12000;
  ...
  _initWebsocketManager() async {
    ...
    BetterSocket.addListener(onOpen: (httpStatus, httpStatusMessage) {
      Log.i("onOpen---httpStatus:$httpStatus  httpStatusMessage:$httpStatusMessage", tag: RoomRecordingPage.sName);
      _socketPingTimer?.cancel();
      _socketPingTimer = null;
      _socketPingTimer = Timer.periodic(_socketPingDuration, (timer) {
        Log.d('sendMsg : ping', tag: RoomRecordingPage.sName);
        BetterSocket.sendMsg('{"Ping":{}}');
      });
    }, onMessage: (message) {
      Log.d('newMsg : $message', tag: RoomRecordingPage.sName);
      try {
        SocketMsg socketMsg = SocketMsg.fromJson(jsonDecode(message.toString()));
        _handleSocketMsg(socketMsg);
      } catch (e) {
        Log.e("_handleSocketMsg ${e.toString()} ", tag: RoomRecordingPage.sName);
      }
    }, onClose: (code, reason, remote) {
      Log.f("onClose---code:$code  reason:$reason  remote:$remote", tag: RoomRecordingPage.sName);
      Future.delayed(Duration(milliseconds: 500), () async {
        BetterSocket.connentSocket(AddressUtil.getInstance().socketUrl(key, widget.roomData.room.roomUuid));
      });
    }, onError: (message) {
      Log.w("IOWebSocketChannel onError $message ", tag: RoomRecordingPage.sName);
    });
    BetterSocket.connentSocket(AddressUtil.getInstance().socketUrl(key, widget.roomData.room.roomUuid));
...
//        break;
  ...
  _handleCourseware(SocketMsg socketMsg) async {
    ...
    Log.d("1111111111111 ${msg["time"]}", tag: RoomRecordingPage.sName);
    ...
    if (!isCheck || flag) {
      Navigator.push(context, PopRoute(child: RedRain())).then((value) {
        ...
        RoomDao.rewardStar(param).then((res) {
          Log.f("rewardStar  ${res.toString()}", tag: RoomRecordingPage.sName);
          if (res.result != null && res.data != null && res.data["code"] == 200) {
...
//      (op as List).addAll(op as List);
      ...
      if (isShow == 0) {
        ...
        Log.f("RoomDao.quesTop param $param", tag: RoomRecordingPage.sName);
        final res = await RoomDao.quesTop(param);
        if (res.result != null && res.data != null && res.data["code"] == 200) {
          Log.f(res.data, tag: RoomRecordingPage.sName);
          if (res.data["data"] != null && res.data["data"].length > 0) {
            showLiveTopDialog(res.data["data"]);
          } else {
            Log.i("该题目无人上榜 ${res.data}", tag: RoomRecordingPage.sName);
          }
        } else {
          Log.e("查询题目前三排行榜失败！！", tag: RoomRecordingPage.sName);
          showToast("网络错误!!!");
        }
      }
    }
  ...
  @override
  String tagName() {
    return RoomRecordingPage.sName;
  }
