...
class RoomLandscapePageState extends State<RoomLandscapePage> with SingleTickerProviderStateMixin {
  ...
  _buildRank(){
    return Align(
      ...
      child: LiveRankWidget(liveCourseallotId: widget.roomData.liveCourseallotId,roomId: widget.roomData.room.roomUuid)
    );
  }
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  Container(
                    ...
                    child: CommonUtils.buildBtn("提交",width: ScreenUtil.getInstance().getWidthPx(130), height: ScreenUtil.getInstance().getHeightPx(250), onTap: () {
                      if (model.val < 1){
                        return;
                      }
                      var isRight = "F";
                      ...
                      final val = model.val;
                      final op  = model.op;
                      var answer = '{"answer":"${model.val-1}","isRight":"$isRight","quesId":${model.ques.qid},"userAnswer":{"qid":${model.ques.qid},"uan":[{"an":"${model.val-1}","r":"$isRight"}]}}';
                      var useTime =  DateTime.now().difference(model.dateTime).inMilliseconds;
                      var param = {"catalogId":model.ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":1,"quesId":model.ques.qid,"isRight":isRight,"roomId":widget.roomData.room.roomUuid};
                      param["answerJson"] = answer;
                      param["useTime"] = useTime;

                      var qlibParam = {"lessonId":model.ques.ctatlogid,"quesId":model.ques.qid,"isRight":isRight,"batchNo":widget.roomData.room.roomUuid,"ctkDatafrommapId":49};
                        ...
                        if (res.result!=null  && res.data !=null && res.data["code"]==200){
                          NavigatorUtil.showGSYDialog(
                              ...
                              builder: (BuildContext context) {
                                print("  op     $op");
                                return LiveQuesDialog(ir: isRight,star:  res.data["data"]["star"],an: "${(op!=null&&op.length==2)?op[val-1]:_selBtnName[val-1]}",);
                              });
                        }else {
                          showToast("提交失败请稍后重试!!!");
                        }
                ...
                child: Container(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text("老师正赶过来，请准备好纸笔耐心等待~",style: TextStyle(color: Colors.orange),)
                      ],
                    )
                )
              ),
...
//                    width:ScreenUtil.getInstance().getWidthPx(200),
        ...
        result = GestureDetector(
          ...
          child: Container(
              width: ScreenUtil.getInstance().screenWidth*0.8,
              child:  AgoraRenderWidget(resModel.res.screenId)
          ),
          onTap:()=>_roomShowTopProvider.setIsShow(true),
        );
        ...
        if(resModel.res.type== h5){
          ...
          result = Container(
            width: ScreenUtil.getInstance().screenWidth*0.8,
            child: CoursewareVideo(flickManager,pic: "${widget.roomData.courseware.localPath}/${resModel.res.data.ps.pic}",soundAction: ()=>_roomShowTopProvider.setIsShow(true)),
          );
        }
      }
          ...
          child: Stack(
            ...
            children: <Widget>[
              AnimatedSwitcher(
                ...
                child: result!=null?Container(key:ValueKey("${resModel?.res?.qid??0}"),child: result,):Container(),
              ),
...
//              color: Colors.red,
          ...
          child:  flag ? AgoraRenderWidget(teacherModel.user.uid):Container(
            color:Colors.white,
            child: Image.asset(
...
//                    height: ScreenUtil.getInstance().getWidthPx(200),
  ...
  Widget _buildWhiteboard()  {
    if (_whiteboard==null){
      _whiteboard = Whiteboard(appIdentifier: "w0MeEJFIEeqZsrtGYadcXg/CnI3nUtyIcYaNg",uuid: widget.boardId,roomToken: widget.boardToken,controller: _whiteboardController);
    }
    return Stack(
        alignment: AlignmentDirectional(0.9, 0.9),
        children: <Widget>[
  ...
  ///聊天区域
  Widget _buildChat(){
    double fontSize = ScreenUtil.getInstance().getSp(6);
...
//   socket.connect();
 ...
 _handleSocketMsg(SocketMsg socketMsg){
  ...
  _handleREDRAIN(text,{isCheck = false}) async {
    ...
    if (isCheck){
      //检查是否已经领奖
      var param = {"catalogId":ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":4,"quesId":ques.qid,"roomId":widget.roomData.room.roomUuid};
    ...
    if(!isCheck || flag){
      Navigator.push(context, PopRoute(child:RedRain())).then((value){
        var param = {"catalogId":ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":4,"quesId":ques.qid,"star":value,"roomId":widget.roomData.room.roomUuid};
  ...
  _handleRANRED(text,{isCheck = false}) async {
    ...
    if (isCheck){
      //检查是否已经领奖
      var param = {"catalogId":ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":3,"quesId":ques.qid,"roomId":widget.roomData.room.roomUuid};
      final res  = await RoomDao.isRewardStar(param);
      if (res.result!=null  && res.data !=null && res.data["code"]==200){
        if (!res.data["data"]["is"]){
          flag = true;
    ...
    if(!isCheck || flag){
      showRedPacketDialog().then((value){
        if (value!=null){
          var param = {"catalogId":ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":3,"quesId":ques.qid,"star":value,"roomId":widget.roomData.room.roomUuid};
          RoomDao.rewardStar(param).then((res){
            Log.f("rewardStar  ${res.toString()}",tag: RoomLandscapePage.sName);
            if (res.result!=null  && res.data !=null && res.data["code"]==200){
              if (res.data["data"]["status"]==1){
  ...
  _handleSEL(text,{isCheck = false}) async {
    ...
    if (isCheck && isShow == 1){
      //检查是否已经领奖
      var param = {"catalogId":ques.ctatlogid,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":1,"quesId":ques.qid,"roomId":widget.roomData.room.roomUuid};
...
//      (op as List).addAll(op as List);
  ...
  Future rewardFirstStar() async {
    ...
    var s = Random().nextInt(15)+10;
      var param = {"catalogId":widget.roomData.courseware.id,"liveCourseallotId":widget.roomData.liveCourseallotId,"liveEventId":2,"star":s,"roomId":widget.roomData.room.roomUuid};
      RoomDao.rewardStar(param).then((res){
        Log.f("rewardStar  ${res.toString()}",tag: RoomLandscapePage.sName);
        if (res.result!=null  && res.data !=null && res.data["code"]==200){
          if (res.data["data"]["status"]==1){
