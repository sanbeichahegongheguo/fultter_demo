...
import 'package:flutter_start/common/redux/middleware/epic.dart';
import 'package:flutter_start/common/redux/middleware/epic_store.dart';
...
class EpicMiddleware<State> extends MiddlewareClass<State> {
  ...
  call(Store<State> store, dynamic action, NextDispatcher next) {
    if (!_isSubscribed) {
      _epics.stream.transform<dynamic>(new SwitchMapStreamTransformer<Epic<State>, dynamic>((epic) => epic(_actions.stream, new EpicStore(store)))).listen(store.dispatch);
