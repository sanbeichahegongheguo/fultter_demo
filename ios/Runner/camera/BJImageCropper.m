...
- (UIImage *)viewToImage:(UIView *)view{  //UIView转换成UIImage
...
- (UIImage *)toCropImage{
    CGRect r = self.crop;
    NSLog(@"Crop location=%f,%f,%f,%f",r.origin.x,r.origin.y,r.size.width, r.size.height);
    
    CGImageRef cgRef = self.image.CGImage;
    CGImageRef imageRef = CGImageCreateWithImageInRect(cgRef, CGRectMake(r.origin.x, r.origin.y, r.size.width, r.size.height));
...
- (CGRect)calcFrameWithImage:(UIImage*)image andMaxSize:(CGSize)maxSize {
    CGFloat increase = 0;//IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE * 2; //origin
...
- (id)initWithImage:(UIImage*)newImage andMaxSize:(CGSize)maxSize {
    ...
    if (self) {
        ...
        //imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE, IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE)]; //origin框架缩小
        imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, 0, 0)]; //移除原先框架缩小，以原图片大小展示
...
- (UIImage*) getFinishImage {
    ...
    // draw image
    //[self.image drawInRect:rect]; //origin
    [[self toCropImage] drawInRect:rect]; //获取裁剪框内容，输出在矩形内
